...
def generate_dense_poster(title, subtitle, n_shapes, palette, max_wobble,
    ...
    """Dense pastel Solar Bloom style poster with larger coverage."""
    if seed is not None:
        np.random.seed(seed)
        random.seed(seed)
    ...
    # Increase density and enlarge size
    for _ in range(n_shapes * 8):  # denser
        color = random.choice(palette)
        alpha = random.uniform(*alpha_range)
        radius = random.uniform(*size_range) * 1.6  # larger size multiplier
        wobble = random.uniform(0.0, max_wobble)
        spikes = random.randint(5, 9)

        # tighter clustering, slightly central bias
        x_center = random.gauss(0, 2.5)
        y_center = random.gauss(0, 2.5)
...
size_min, size_max = st.sidebar.slider("Heart Size (Radius)", 0.2, 0.8, (0.25, 0.5))
...
st.sidebar.download_button(
    ...
    data=buf.getvalue(),
    file_name="generative_poster_dense_large.png",
    mime="image/png"
)
