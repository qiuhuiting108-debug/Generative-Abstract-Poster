...
def pastel_palette():
    return [
        (0.98, 0.85, 0.72, 0.42),  # soft yellow
        (0.94, 0.78, 0.80, 0.42),  # pink
        (0.75, 0.82, 0.89, 0.42),  # light blue
        (0.89, 0.71, 0.82, 0.42),  # purple-pink
        (0.96, 0.78, 0.65, 0.42),  # peach
        (0.88, 0.79, 0.89, 0.42),  # lilac
    ]
...
def generate_poster(seed=None):
    ...
    # draw more open, balanced layout
    for i in range(85):
        color = random.choice(palette)
        ec_color = (0, 0, 0, 0.25)  # subtle gray outline
        r = random.uniform(1.3, 2.6)  # smaller shapes
        x_center = random.uniform(-4, 4)  # ✳️ 更大范围分布
...
with st.sidebar:
    ...
    num_layers = st.slider("Number of Layers", 50, 150, 85)
...
if generate:
    ...
    st.download_button("💾 Download Poster (PNG)",
                       data=buf.getvalue(),
                       file_name="Week2_SolarBloom_Outlined_Spread.png",
                       mime="image/png")
else:
